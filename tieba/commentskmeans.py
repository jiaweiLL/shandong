...
for i in range(1, ncols):
    for j in range(1, nrows):
        ...
        l = len(b)
        for k in range(l):
            text_list.append(b[k])
print(text_list)
tfidf_matrix = tfidf_vectorizer.fit_transform(text_list)

num_clusters = 20
km_cluster = KMeans(n_clusters=num_clusters, max_iter=1000, n_init=5, \
                    init='k-means++',n_jobs=2)
'''
n_clusters: 指定K的值
...
wc.to_file("WordCould_comments.png")
...
with open("comment_word.csv", 'w', newline="") as f:
    writer = csv.writer(f)
